...
const StaffSignIn = (props) => {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await props.staffLogin(email, password);
    } catch {
  ...
  return (
    <Fragment>
      <form id="staff-signin-form" onSubmit={onSubmit} noValidate>
        ...
        >
          {loading ? <><span className="ds-spinner" style={{ borderColor: "rgba(255,255,255,0.4)", borderTopColor: "#fff" }} /> Signing in…</> : "Sign In as Staff"}
        </button>
      </form>
    </Fragment>
  );
